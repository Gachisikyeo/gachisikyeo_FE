...
export default function GroupPurchaseCreateModal({
  ...
  const eachPrice = useMemo(() => {
    if (typeof eachPriceFromBE === "number" && eachPriceFromBE > 0)
      return eachPriceFromBE;
  ...
  const handlePay = async (method: "NAVER" | "KAKAO") => {
    ...
    try {
      ...
      const USE_MOCK = true;
      if (USE_MOCK) {
        console.log("[MOCK] createGroupPurchase", {
          productId,
          body,
          shippingAddress,
          phone,
      ...
      >
        ...
        <div className="gp-productTop">
          ...
          <div className="gp-productPrice">
            {eachPrice.toLocaleString("ko-KR")}/{packagePrice.toLocaleString("ko-KR")}
            원
          </div>
        </div>
          ...
          <div className="gp-row2">
            ...
            <div className="gp-field">
              ...
              {!pickupAtOk && (pickupHour.length > 0 || pickupMinute.length > 0) && (
                <div className="gp-warnInline">
                  시간을 올바르게 입력해주세요 (시: 1~12 / 분: 00~59)
                </div>
              )}
            </div>
          </div>
