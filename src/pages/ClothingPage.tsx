...
function ClothingPage() {
  ...
  const fetchPage = async (nextPage: number, append: boolean) => {
    const res = await getProductsByCategory("CLOTHES", {
      page: nextPage,
      size: PAGE_SIZE,
      sortKey: "CREATED_AT",
      direction: "DESC",
  ...
  useEffect(() => {
    const init = async () => {
      ...
      try {
        ...
        console.error("clothes products fetch failed:", e);
        setProducts([]);
        setHasNext(false);
  ...
  const handleLoadMore = async () => {
    ...
    try {
      ...
      console.error("clothes products load more failed:", e);
    } finally {
  ...
  return (
    <div>
      ...
      <main className="app-layout page-list" style={{ paddingBottom: "40px" }}>
        <ProductSection title="" products={sorted} onClickViewMore={() => {}} onClickProduct={handleClickProduct} />
